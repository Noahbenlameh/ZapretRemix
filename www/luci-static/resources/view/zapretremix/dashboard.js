	...
	render: function (data) {
		...
		var container = E('div', { 'class': 'cbi-map' }, [
			...
			E('p', { 'class': 'cbi-value-description' },
				...
				'(так было обнаружено с youtube.com в этой сети). Переключение DNS обходит именно это — отдельно от всего остального выше.'),

			E('div', { 'style': 'margin:14px 0;padding:12px 14px;background:rgba(120,140,160,0.08);border-radius:4px;font-size:12.5px;line-height:1.6;' }, [
				E('p', { 'style': 'margin:0 0 8px;font-weight:bold;' }, 'Что выбрать:'),
				E('p', { 'style': 'margin:4px 0;' }, [ E('strong', {}, 'DNS провайдера (авто) '), '— как есть по умолчанию, DHCP от провайдера. Меняется сам, если провайдер поменяет сервер. Используй, если проблем с DNS нет.' ]),
				E('p', { 'style': 'margin:4px 0;' }, [ E('strong', {}, 'Публичный DNS (8.8.8.8, 1.1.1.1) '), '— фиксированные надёжные сервера Google/Cloudflare. Обходит подмену DNS провайдером. Используй, если сайт не резолвится/резолвится не туда.' ]),
				E('p', { 'style': 'margin:4px 0;' }, [ E('strong', {}, 'DNS провайдера по IP '), '— то же, что "авто", но зафиксированный конкретный IP, а не "что бы ни выдал DHCP сейчас". Полезно для диагностики (в "Рекомендациях" именно так проверяется, что реально блокирует провайдер) и если хочешь остаться на DNS провайдера, но без риска, что он внезапно подменится через DHCP.' ]),
				E('p', { 'style': 'margin:4px 0;' }, [ E('strong', {}, 'Свой DNS '), '— вручную указанные сервера (например, DNS друга, другого провайдера, любой резолвер на твой выбор).' ])
			]),
